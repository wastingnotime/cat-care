...
export default function Today() {
  ...
  const jumpTo = (event: MouseEvent, section: string, input: string) => {
  ...
  return <Shell>
    ...
    <section class="quick-actions" aria-labelledby="quick-actions-title"><div><p class="eyebrow">Quick actions</p><h2 id="quick-actions-title">Record care</h2></div><div class="quick-action-links"><a class="secondary" href="#observations" onClick={event => jumpTo(event, "observations", "observation-input")}>Add observation</a><a class="secondary" href="#direct-care" onClick={event => jumpTo(event, "direct-care", "care-description-input")}>Record direct care</a></div></section>
    ...
    </article><aside class="panel history-panel"><div class="panel-heading"><div><p class="eyebrow">Care history</p><h2>Recent activity</h2></div></div><ol class="timeline"><For each={(timeline()??[]).slice(0,10)}>{item=><li><strong>{item.description}</strong><span>{item.type.replaceAll("_"," ")}</span><time dateTime={item.occurred_at}>{formatDate(item.occurred_at)}</time></li>}</For></ol></aside></section>
    <section class="feature-grid"><article class="panel" id="observations"><p class="eyebrow">Observations</p><h2>Notes, not diagnoses</h2><form class="stack-form" onSubmit={recordNote}><label>What did you notice?<textarea id="observation-input" value={noteText()} onInput={e=>setNoteText(e.currentTarget.value)} required placeholder="Eating less than usual"/></label><button class="primary">Record observation</button></form><ul class="record-list"><For each={(notes()??[]).slice().reverse()}>{note=><li><span>{note.description}</span><button class="text-button" onClick={()=>requestTriage(note.id)}>Request triage</button></li>}</For></ul></article>
      <article class="panel" id="direct-care"><p class="eyebrow">Direct care</p><h2>Record what happened</h2><form class="stack-form" onSubmit={recordCare}><label>Type<select value={careType()} onChange={e=>setCareType(e.currentTarget.value)}><option value="measurement">Measurement</option><option value="exam_performed">Exam</option><option value="medication_given">Medication given</option></select></label><label>Description<textarea id="care-description-input" value={careDescription()} onInput={e=>setCareDescription(e.currentTarget.value)} required placeholder="Weight 4.2 kg"/></label><button class="primary">Record care event</button></form></article>
    </section>
    <Show when={pendingTriage().length > 0}><section class="panel triage-summary"><div><p class="eyebrow">Review status</p><h2>{pendingTriage().length} assessment{pendingTriage().length === 1 ? "" : "s"} sent for veterinarian review</h2><p>Your veterinarian has not completed the review yet.</p></div></section></Show>
  </Shell>;
}
