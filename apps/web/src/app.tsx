...
function AuthBoundary(props:{children:unknown}) {
  const location=useLocation(); const [session]=createResource(()=>location.pathname!=="/login", enabled=>enabled?api<Session>("session").catch(error=>{if(error instanceof ApiError && error.status===401)return null;throw error}):undefined);
