...
export function Shell(props: { children: JSX.Element }) {
  ...
  return <>
    <header class="site-header">
      ...
      <div class="session-controls">{(session()?.user.roles.length??0)>1?<button class="workspace-switch" type="button" role="switch" aria-checked={session()?.user.mode==="veterinarian"} aria-label={`Switch to ${session()?.user.mode==="veterinarian"?"owner":"veterinarian"} workspace`} data-mode={session()?.user.mode} onClick={()=>switchWorkspace(session()?.user.mode==="veterinarian"?"owner":"veterinarian")}><span>Owner</span><span>Veterinarian</span><i aria-hidden="true"/></button>:<span class={`mode-badge ${session()?.user.mode}`}>{session()?.user.mode??"local"}</span>}<button class="text-button" onClick={async()=>{await api("session",{method:"DELETE"});window.location.href="/login"}}>Log out</button></div>
    </header>
