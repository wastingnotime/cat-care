  ...
  return <>
    <header class="site-header">
      ...
      <nav class="primary-nav" aria-label="Primary navigation">
        {navigation.filter(item=>session()?.user.mode!=="veterinarian" || item.href==="/triage").map(item => <A href={item.href} aria-current={isCurrent(item.href) ? "page" : undefined}>{item.label}</A>)}
      </nav>
